...
let Database = class {
    ...
    insertPost(title, text, images, phone, name, category, status) {
        const promise = new Promise((resolve, reject) => {
            this.con.query('INSERT INTO `posts` (`title`, `text`, `images`, `phone`, `name`, `category`, `status`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?, NOW());', [title, text, images, phone, name, category, status], function (error, results, fields) {
                if (error) throw error;
